...
func TestSettingsHelper_GetIDFromURL(t *testing.T) {
	...
	Convey("GetIDFromURL", t, func() {
		...
		Convey("should return valid id when passed valid URL", func() {
			id, err := source.GetIDFromURL("https://yenpress.com/series/is-it-wrong-to-try-to-pick-up-girls-in-a-dungeon-light-novel")

			So(id, ShouldResemble, "is-it-wrong-to-try-to-pick-up-girls-in-a-dungeon-light-novel")
			So(err, ShouldBeNil)
		})
